...
> Подробное описание возможностей каждого сервера — в [официальной документации](https://docs.onerpa.ru/mcp-servery-1c).

### Описание серверов

| Имя в конфигурации | Порт | Назначение | |
|---------------------|------|------------|---|
| `1c-help` | 8003 | Справка по платформе 1С для вашей версии | рекомендуется |
| `1c-graph-metadata` | 8006 | Графовый поиск по метаданным конфигурации (Neo4j) | рекомендуется |
| `1c-metadata` | 8000 | Поиск по метаданным и коду модулей (ChromaDB) | рекомендуется |
| `1c-syntax` | 8002 | Проверка синтаксиса кода (BSL Language Server) | рекомендуется |
| `1c-ssl` | 8008 | Справка по БСП (Библиотека стандартных подсистем) | если используете БСП |
| `1c-templates` | 8004 | Шаблоны кода (публичные + собственные) | |
| `1c-codechecker` | 8007 | Проверка кода через 1С:Напарник | |
| `1c-forms` | 8011 | Контекст для генерации управляемых форм | |

### Минимальная конфигурация

Для начала работы достаточно подключить 4 основных сервера. Остальные можно добавить позже по необходимости.
...
Все доступные серверы (см. таблицу выше для описания каждого):
